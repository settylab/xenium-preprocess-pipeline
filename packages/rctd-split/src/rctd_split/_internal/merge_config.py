...
Merge invariant (from (internal issue review): each
pipeline reads the current `config.yaml`, writes back ONLY its
own top-level key (`xenium_preprocess:` / `ref_build:` / `rctd_split:` /
`driver:`), and PRESERVES all sibling top-level keys. This lets the three
pipelines run in sequence under a shared `--run-id` folder without any
pipeline clobbering another's section.
...
identical copy of this file under their own `_internal/`. Tests in each
repo cover the invariant so a regression in any one pipeline is caught
locally.
"""
