...
#
# Usage:
#   sbatch scripts/submit.slurm.sh SAMPLE_ID PRIMARY_H5AD [--key value ...]
#
# Example:
...
SAMPLE_ID="${1:?usage: sbatch scripts/submit.slurm.sh SAMPLE_ID PRIMARY_H5AD [extra --flags]}"
PRIMARY_H5AD="${2:?missing PRIMARY_H5AD}"
shift $(( $# < 2 ? $# : 2 ))
EXTRA_ARGS=("$@")

# ----- Resolve OUTPUT_ROOT (mirrors ref-build precedence) -------------
# Precedence: --output-root in EXTRA_ARGS > $OUTPUT_ROOT env > default.
DEFAULT_OUTPUT_ROOT="/data/ref_build_runs"
...
JOB_TAG="${SLURM_JOB_NAME:-ref-build}_${SLURM_JOB_ID:-local-$(date +%Y%m%d-%H%M%S)}"
LOG_FILE="$LOG_DIR/${JOB_TAG}.log"

# ----- Validate paths BEFORE the tee redirect ------------------------
fail=0
if [[ ! -e "$PRIMARY_H5AD" ]]; then
    echo "[submit] ERROR: PRIMARY_H5AD does not exist: $PRIMARY_H5AD" >&2
    fail=1
fi
if [[ $fail -ne 0 ]]; then
...
#
# TWO PATHS, chosen by execution context (ported verbatim from
# xenium-preprocess and the H&E pipeline -- see comment in
# xenium-preprocess/scripts/submit.slurm.sh for the deadlock root
# cause that motivates the split):
#
...
echo "[submit] which ref-build:  $(command -v ref-build 2>/dev/null || echo 'NOT FOUND — did you `pip install -e .` inside the env?')"
echo "[submit] CONDA_PREFIX:     ${CONDA_PREFIX:-unset}"

# ----- LMOD R for the rctd_reference_build stage ---------------------
# Stage 5 shells out to Rscript. The reference ref-build recipe (Stage C in
# the summary) uses the Lmod R module `fhR/4.4.1-foss-2023b`
# which carries Seurat + Matrix + readr preinstalled. `spacexr` (RCTD)
# is NOT in that module — see docs/install.md for the one-time
# user-local install into ~/.claude/r_libs/4.4.1. The R script prepends
# that path to .libPaths() on startup.
#
...
if command -v ml >/dev/null 2>&1; then
    ...
    ml "$R_MODULE" || echo "[submit] WARN: 'ml $R_MODULE' failed; rctd_reference_build may fail unless Rscript is otherwise on PATH."
elif command -v module >/dev/null 2>&1; then
    module load "$R_MODULE" || echo "[submit] WARN: 'module load $R_MODULE' failed; rctd_reference_build may fail."
else
    echo "[submit] WARN: no 'ml' / 'module' on PATH. rctd_reference_build depends on Rscript; make sure it's reachable."
fi
echo "[submit] which Rscript:    $(command -v Rscript 2>/dev/null || echo 'NOT FOUND — rctd_reference_build will fail')"
...
# Point Python at the pipeline source (no `pip install -e .` required).
...
export PYTHONPATH="<path-to-xenium-preprocess-pipeline>/packages/ref-build/src"

echo "[submit] running:"
echo "    python -u -m ref_build.cli ${ARGS[*]}"
...
"${CONDA_PREFIX:-~/micromamba/envs/xenium}/bin/python" -u -m ref_build.cli "${ARGS[@]}"
