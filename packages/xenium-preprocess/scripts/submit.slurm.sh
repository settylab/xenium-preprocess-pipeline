...
#
# Usage:
#   sbatch scripts/submit.slurm.sh SAMPLE_ID PROSEG_DIR [--key value ...]
#
# Example:
...
# ---------------------------------------------------------------------
#SBATCH --job-name=xenium-preprocess
...
SAMPLE_ID="${1:?usage: sbatch scripts/submit.slurm.sh SAMPLE_ID PROSEG_DIR [extra --flags]}"
PROSEG_DIR="${2:?missing PROSEG_DIR}"
shift $(( $# < 2 ? $# : 2 ))
EXTRA_ARGS=("$@")

# ----- Resolve OUTPUT_ROOT (mirrors xenium-preprocess precedence) -----
# Precedence: --output-root in EXTRA_ARGS > $OUTPUT_ROOT env > default.
DEFAULT_OUTPUT_ROOT="/data/xenium_preprocess_runs"
...
JOB_TAG="${SLURM_JOB_NAME:-xenium-preprocess}_${SLURM_JOB_ID:-local-$(date +%Y%m%d-%H%M%S)}"
LOG_FILE="$LOG_DIR/${JOB_TAG}.log"

# ----- Validate paths BEFORE the tee redirect ------------------------
fail=0
if [[ ! -e "$PROSEG_DIR" ]]; then
    echo "[submit] ERROR: PROSEG_DIR does not exist: $PROSEG_DIR" >&2
    fail=1
fi
if [[ $fail -ne 0 ]]; then
...
#
# TWO PATHS, chosen by execution context (ported verbatim from the
# H&E pipeline's submit.sh -- see comment there for the deadlock root
...
echo "[submit] which xenium-preprocess: $(command -v xenium-preprocess 2>/dev/null || echo 'NOT FOUND — did you `pip install -e .` inside the env?')"
echo "[submit] CONDA_PREFIX: ${CONDA_PREFIX:-unset}"

# ----- LMOD R for the rctd_prep stage --------------------------------
# The rctd_prep stage shells out to Rscript. The reference workflow
# uses the Lmod R module `fhR/4.4.1-foss-2023b` which carries
# Seurat + spacexr + Matrix + SpatialExperiment preinstalled.
#
...
if command -v ml >/dev/null 2>&1; then
    ...
    ml "$R_MODULE" || echo "[submit] WARN: 'ml $R_MODULE' failed; rctd_prep may fail unless Rscript is otherwise on PATH."
elif command -v module >/dev/null 2>&1; then
    module load "$R_MODULE" || echo "[submit] WARN: 'module load $R_MODULE' failed; rctd_prep may fail."
else
    echo "[submit] WARN: no 'ml' / 'module' on PATH. rctd_prep depends on Rscript; make sure it's reachable."
fi
echo "[submit] which Rscript: $(command -v Rscript 2>/dev/null || echo 'NOT FOUND — rctd_prep will fail')"
...
export PYTHONPATH="<path-to-xenium-preprocess-pipeline>/packages/xenium-preprocess/src"

echo "[submit] running:"
echo "    python -u -m xenium_preprocess.cli ${ARGS[*]}"
...
"${CONDA_PREFIX:-~/micromamba/envs/xenium}/bin/python" -u -m xenium_preprocess.cli "${ARGS[@]}"
